...
func gameloop(win *pixelgl.Window) {
	...
	for !win.Closed() {
		...
		gopherimg.Draw(win,
			pixel.IM.Moved(win.Bounds().Center()).Scaled(win.Bounds().Center(), 4))
...
func run() {
	...
	f, err := os.Open("assets/emoji-3X.png")
	if err != nil {
		panic(err)
	}
	...
	//mat = mat.Scaled(win.Bounds().Center(), 88)
	//gopherimg.Draw(win, pixel.IM)
	gameloop(win)
}
